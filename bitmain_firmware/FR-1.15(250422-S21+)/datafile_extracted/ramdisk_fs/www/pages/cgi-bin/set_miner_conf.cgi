...
{
    config_file="/config/cminer.json"
    echo "{"                                                            > $config_file
...
if [[ $? -eq 0 ]]&&[[ ! "$QUERY_STRING_POST"x == ""x ]]; then
    ...
    if [[ "$ret"x == "null"x ]]||([[ $ret -ne 0 ]]&&[[ $ret -ne 1 ]]&&[[ $ret -ne 2 ]]&&[[ $ret -ne 3 ]]);then
...
#   fi
    ...
    if [ $reload == true ];then
        api {\"command\":\"reload\",\"new_api\":true} > /dev/null &
    else
        sudo /etc/init.d/S70cminer restart > /dev/null &
