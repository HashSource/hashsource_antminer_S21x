...
{
    # 检查CONTENT_TYPE是否为application/x-www-form-urlencoded或application/json
    # [ "${CONTENT_TYPE}" != "application/x-www-form-urlencoded" ] && \
    # echo "Warning: you should probably use MIME type "\
    #      "application/x-www-form-urlencoded!" 1>&2
    if [ "${CONTENT_TYPE}" = "application/x-www-form-urlencoded" ]; then
        echo "Received URL encoded data."
    elif [ "${CONTENT_TYPE}" = "application/json" ]; then
        echo "Received JSON data."
    else
        echo "Warning: Unknown or unsupported MIME type: ${CONTENT_TYPE}" 1>&2
        return 1  # 返回错误码，表示不支持的类型
    fi
    
    # 检查是否为POST请求且CONTENT_LENGTH不为空
    [ -z "$QUERY_STRING_POST" \
...
function update_config_file()
{
    ...
    echo "\"bitmain-free-power\" : \"$ant_free_power\","                >> $config_file
...
if [[ $? -eq 0 ]]&&[[ ! "$QUERY_STRING_POST"x == ""x ]]; then
    ...
    if [[ "$ret"x == "null"x ]]||([[ $ret -ne 0 ]]&&[[ $ret -ne 1 ]]&&[[ $ret -ne 2 ]]&&[[ $ret -ne 3 ]]&&[[ $ret -ne 8 ]]);then
    ...
    ret=`parse_configs $QUERY_STRING_POST '."free-power"'`
    echo "free power from web is $ret"
    if [ "$ret"x == "null"x ];then
        ant_free_power=`old_configs '."bitmain-free-power"'`
    else
        # old_ant_freq_power=`old_configs '."bitmain-free-power"'`
        # if [ $old_ant_freq_power == $ret ];then
        #     reload=true
        # fi
        ant_free_power=$ret
    fi
